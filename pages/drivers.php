...
<body>
    ...
    <div class="content">
        ...
        <table class="table">
            <tr>
                <th class="col">UID</th>
                <th class="col">Contact Details</th>
                <th class="col">Name</th>
                <th class="col">Discount</th>
                <th class="col">Driver Info</th>
            ...
            for ($i = 1; $i < 11; $i++) {
                ...
                echo <<<HTML
                <tr>
                    <td class="col"><small>{$faker->uuid()}</small></td>
                    <td class="col">{$faker->phoneNumber()}<br /><small class="d-block">{$faker->email()}</small></td>
                    <td class="col">{$faker->name()}</td>
                    <td class="col">{$faker->randomElement(['None', 'Applying', 'Student', 'Senior Citizen', 'PWD'])}</td>
                    <td class="col">
                        {$faker->randomElements(['Tricycle', 'Jeepney', 'Modern Jeepney', 'Bus', 'Other PUVs'])[0]}<br>
                        <small class="d-block">Dagupan City - {$faker->randomElements(['Downtown Loop', 'Bonuan Tondaligan', 'Bonuan Boquig', 'Salisay/Bolosan', 'CSI Lucao'])[0]}</small>
                    </td>
                    <td class="col">
                        <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#modal"><i class="fa-solid fa-magnifying-glass"></i></button>
                        <button type="button" data-type="WARNING" class="btn btn-action btn-outline-warning"><i class="fa-solid fa-lock"></i></button>
                        <button type="button" data-type="DANGER" class="btn btn-action btn-outline-danger"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
                HTML;
            }
                        ...
                        echo <<<HTML
                            ...
                            <div class="d-flex">
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" value="{$faker->date($format = 'm/d/Y', $max = '-18 years')}">
                                </div>
                            </div>
                            <div class="d-flex">
                                <div>
                            ...
                            <div class="d-flex">
                                ...
                                <div>
                                    ...
                                    <input type="text" class="form-control" value="{$faker->randomElement(['Approved', 'Updating'])}">
                                </div>
                            </div>
                            <div class="d-flex">
                                <div>
                                    <label for="">Plate Number</label>
                                    <input type="text" class="form-control" value="{$faker->randomLetter()}{$faker->randomLetter()}{$faker->randomLetter()} {$faker->randomNumber(3)}">
                                </div>
                                <div>
                                    <label for="">PUV Type</label>
                                    <input type="text" class="form-control" value="{$faker->randomElements(['Tricycle', 'Jeepney', 'Modern Jeepney', 'Bus', 'Other PUVs'])[0]}">
                                </div>
                                <div>
                                    <label for="">Route</label>
                                    <input type="text" class="form-control" value="Dagupan City - Downtown Loop">
                                </div>
                            </div>
                            <div class="d-flex">
                                <div>
                                    <label for="">Driver's License</label>
                                    <img src="{$faker->imageUrl(360, 360, 'Driver\'s', false, 'License', true, 'png')}" alt="">
                                </div>
                                <div>
                                    <label for="">Jeepney Registration</label>
                                    <img src="{$faker->imageUrl(360, 360, 'Jeepney', false, 'Registration', true, 'png')}" alt="">
                                </div>
                            </div>
                        HTML;
                        ?>
...
<script>
    $("#navDrivers").addClass("active");
    $("#navDrivers").removeClass("link-dark");
</script>
<script type="module">
    import {
        Toaster,
    ...
    for (const btn of btns) {
        ...
        btn.addEventListener("click", (e) => {
            var msg = (type == "WARNING") ? "Successfully Disabled Driver Application!" : "Successfully Deleted Driver Application";
